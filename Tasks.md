...
- [ ] `scan()` перебирает **прямые подпапки** `VIDEO_ROOT` (`video_root.iterdir()`, только `is_dir()`) — это и есть «группы»; имя подпапки как есть (кириллица) идёт в `VideoFile.group_folder`.
- [ ] Внутри каждой папки группы — **только файлы** (`is_file()`), без рекурсии; это автоматически отсекает `_uploaded` и любые другие вложенные каталоги (решение 4 выше).
- [ ] Фильтр по имени: `name.startswith(".")` или `name.startswith("~")` → пропустить.
- [ ] Фильтр по расширению: `path.suffix.lower() in allowed_extensions` (расширения в `Settings.allowed_extensions` уже нормализованы лишь при чтении из env — но сравнивать всё равно нужно регистронезависимо, на диске файл может быть `REC.WEBM`).
- [ ] На каждый прошедший фильтры файл — `path.stat()` один раз, собрать `VideoFile(path=path, group_folder=<имя папки>, size_bytes=stat.st_size, mtime=stat.st_mtime)`.
- [ ] `video_root.iterdir()` и обход одной группы — в `try/except OSError`; при ошибке — `logger.warning(...)` с именем папки и причиной, переход к следующей (решение 5). Если сам `VIDEO_ROOT` не существует/недоступен — это уже не «одна группа», можно дать упасть (пайплайн решит, что делать при старте; не заглушать молча).
- [ ] Итоговый список — `sorted(..., key=lambda vf: vf.mtime)`.
...
- [ ] `path.stat()` → `mtime`; сравнить `datetime.now(UTC) - datetime.fromtimestamp(stat.st_mtime, tz=UTC) >= self._threshold`.
- [ ] Файл должен ещё и открываться на чтение: `with path.open("rb"): pass` в `try/except OSError` → `False` при неудаче (без исключения наружу — это ожидаемая ситуация «подождать», не ошибка).
- [ ] `FileNotFoundError`/`OSError` на `stat()` тоже → `False` (файл мог исчезнуть между scan и проверкой — не должно ронять цикл).
- [ ] Никакого `MAX_ATTEMPTS`/событий/логики ретраев — это не входит в зону ответственности класса (решение 3).

### 4.3 Тесты

`tests/scanner/test_scanner.py` (реальные временные каталоги через `tmp_path`, никаких реальных путей):

- [ ] Базовый кейс: `tmp_path/КЕГЭ-1/rec.webm` и `tmp_path/ОГЭ-1/rec2.webm` (`allowed_extensions=(".webm",)`) → оба найдены, `group_folder` соответствует папке.
- [ ] Файл с расширением не из списка (`.mp3`) → не попадает.
- [ ] Регистр расширения на диске (`REC.WEBM`) при `allowed_extensions=(".webm",)` → находится.
- [ ] Имя, начинающееся с `.` (`.DS_Store.webm`) и с `~` (`~temp.webm`) → пропущены.
- [ ] Файл прямо в `VIDEO_ROOT` (не внутри папки группы) → не считается кандидатом.
- [ ] Вложенная папка `_uploaded/old.webm` внутри группы → не считается кандидатом (глубина 1).
- [ ] Пустая папка группы → просто не даёт кандидатов, без ошибки.
- [ ] Сортировка: три файла с разным `mtime` (выставить через `os.utime`) → `scan()` возвращает их по возрастанию `mtime`.
- [ ] Изоляция ошибок: `monkeypatch.setattr(Path, "iterdir", ...)` — обёртка, которая для одной конкретной папки группы поднимает `OSError`, для остальных вызывает оригинал; проверить, что кандидаты из других папок всё равно вернулись, и что в `caplog` есть `WARNING`.

`tests/scanner/test_stability.py`:

- [ ] Свежесозданный файл (`mtime` = только что) при `stability_minutes=5` → `is_stable() is False`.
- [ ] Файл с `mtime`, состаренным через `os.utime(path, (old_ts, old_ts))` на 10 минут назад при пороге 5 → `True`.
- [ ] Файл ровно на границе порога — зафиксируйте тестом, какое поведение выбрали (`>=` включает границу — так и должно быть по формулировке CLAUDE.md «не менялись ≥ N»).
- [ ] Несуществующий путь → `False`, без исключения.
- [ ] Неоткрываемый файл (замокать `Path.open`, чтобы поднимал `OSError`, даже если `mtime` достаточно старый) → `False`.

### Definition of Done (этап 4)

- [ ] `uv run ruff format . && uv run ruff check . && uv run mypy video_uploader && uv run pytest` — чисто.
- [ ] Ревью Claude пройдено, замечания закрыты.
