...
**Принято 2026-07-22** (советы 3–4): `event` — третий (по факту четвёртый после `logger`)
лейбл Loki-стрима, без единого словаря имён в коде (литеральные строки по месту вызова —
обсуждали `EVENT_NAMES`, отказались как от лишней абстракции на 9 точках). Расширено за
пределы 7 доменных событий на основные внесобытийные логи, нужные для метрик (heartbeat,
старт/стоп, ошибки цикла сканирования и т.п.). Полный актуальный список значений, уровней и
файлов — `.docs/basic_doc.md`, раздел «Метрики и алерты в Grafana»; ход работы —
`.docs/Tasks.md`, «Доп. правка — `event` третьим лейблом Loki-стрима». Синхронизировано с
`fs-adsync` (см. его `Events-Logging.md`/`Tasks.md`). Совет 1 (маркер `matched=true/false`
поверх `event=video_registered`) не делали — вне текущего запроса.