...
5. В `config/groups.yaml` — блок `lms` для каждой папки: `{group_id, course_id, teacher_id}` для групповых занятий или `{teacher_username}` (без `group_id`) для персональной папки преподавателя — плагин сам определяет ветку резолва по составу блока, сервис его не интерпретирует.
