...
#### 5.7. Проверка обоих режимов сухого прогона

Сервис поддерживает два независимых режима dry-run (см. `DRY_RUN`/`DRY_RUN_LMS_LIVE` в `.env.example` и `.docs/CLAUDE.md`, раздел Configuration):

| Режим | `DRY_RUN` | `DRY_RUN_LMS_LIVE` | S3-загрузка | Регистрация в LMS | Архивация исходника |
|---|---|---|---|---|---|
| Полный сухой прогон | `true` | `false` (по умолчанию) | заглушка (лог `dry-run: upload_video пропущен`) | заглушка (лог `dry-run: register пропущен`) | заглушка (лог `dry-run: архивация ... пропущена`) |
| Сухой прогон + реальная регистрация в LMS | `true` | `true` | заглушка | **настоящий HTTP-вызов** в `fs-lms` (HMAC) | заглушка (S3-объекта на самом деле нет, архивировать нельзя) |
| Прод | `false` | не важно | настоящая | настоящая | настоящая (если `ARCHIVE_AFTER_REGISTER=true`) |

##### Режим 1 — `DRY_RUN=true`, `DRY_RUN_LMS_LIVE=false`

Ничего не трогает сеть. Проверка:

1. `.env`: `DRY_RUN=true`, `DRY_RUN_LMS_LIVE=false` (или не задавать — это default).
2. Положить тестовый видеофайл в `VIDEO_ROOT/<группа из groups.yaml>/`.
3. `docker compose up -d` (или `uv run video-uploader` локально) → `curl -X POST http://localhost:8090/rescan`.
4. `curl http://localhost:8090/status` — файл должен дойти до `registered` (архивации не будет — файл остаётся на месте).
5. В логе (`data/logs/uploader.log` или `docker compose logs`) — обе строки `dry-run: upload_video пропущен` и `dry-run: register пропущен (успех)`. Ни одного реального обращения к S3/LMS не было.

Это базовая проверка «сканер правильно видит файлы и матчит их на `groups.yaml`» — годится без доступа к реальному S3/fs-lms вообще.

##### Режим 2 — `DRY_RUN=true`, `DRY_RUN_LMS_LIVE=true`

Проверяет по-настоящему, что fs-lms корректно матчит зарегистрированное видео на занятие по дате/времени — без реальной загрузки видео в S3 (значит, без расхода места в бакете и без риска что-то сломать на проде).

**Что подготовить заранее:**

1. **Тестовый курс/группа в fs-lms** (staging-инсталляция или отдельный тестовый курс на боевой, если staging нет) — узнать её `group_id`/`course_id`/`teacher_id` в админке fs-lms (таблица `fs_lms_groups`).
2. **Тестовая запись в `groups.yaml`** с этими же ID:
   ```yaml
   groups:
     "test-group":
       slug: test-group
       lms:
         group_id: <ID из fs-lms>
         course_id: <ID из fs-lms>
         teacher_id: <ID из fs-lms>
   ```
   Папку `test-group` создать в `VIDEO_ROOT`.
3. **Занятие в расписании этого курса задним числом** — например, вчера в 16:04. Плагин матчит по дате **и времени**, не только по дню — фиксируйте конкретный час/минуту.
4. **Тестовый видеофайл** с именем по паттерну Телемоста, где блок даты/времени совпадает со временем занятия из п. 3:
   ```
   Встреча_в_Телемосте_<вчера_ДД>_<ММ>_<ГГ>_16_04_00_—_запись.webm
   ```
   Это и есть `recorded_at`, который уйдёт в LMS-payload (`.docs/VU_API.md`). Часовой пояс — `TZ_NAME` (по умолчанию `Europe/Kaliningrad`); если занятие в fs-lms в другом TZ, время может не совпасть — тогда `matched: false`, а не ошибка.
5. Положить файл в `VIDEO_ROOT/test-group/`.

**Запуск:**

```bash
DRY_RUN=true
DRY_RUN_LMS_LIVE=true
LMS_BASE_URL=<адрес тестового/staging fs-lms>
LMS_HMAC_SECRET=<секрет, совпадающий с FS_LMS_VIDEO_HMAC_SECRET на этом сайте>
```

`docker compose up -d --build` (или локально) → `POST /rescan` → `GET /status`.

**Проверка результата:**

- В логе — **нет** строки `dry-run: register пропущен`; вместо неё — реальный ответ HTTP (200/201) или предупреждение `LMS: занятие не найдено по дате/времени, оставлено на ручную привязку` (это `matched: false`, тоже штатный `registered`, но матчинг не сработал — проверьте дату/время/ID из подготовки).
- `/status` показывает файл в `registered` (не в `failed`) — `401`/`5xx`/сетевые ошибки от fs-lms попадут в `failed`/ретраи, это тоже полезный сигнал (например, разъехались часы LXC и fs-lms — HMAC-окно ±300 с, см. `.docs/VU_API.md`).
- В самой fs-lms (админка) — занятие из п. 3 должно пометиться проведённым (`held`) и получить привязанную видеозапись с `matched: true`.
